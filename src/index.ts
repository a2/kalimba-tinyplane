...
class Game extends Thumby {
  ...
  intro() {
    ...
    while (!this.input(Thumby.Input.A)) {
      ...
      this.gc.clearScreen();
      if (sy - bitmap.height / 2 < this.gc.getHeight()) {
        this.gc.drawBitmap(sx - bitmap.width / 2, sy - bitmap.height / 2, bitmap);
      } else if (Math.floor(timer / 30) % 2 == 0) {
        const start = 'start';
        const startSize = this.gc.measureText(start);
        this.gc.drawText((this.gc.getWidth() - startSize.width) >> 1, this.gc.getHeight() - startSize.height, start);
      }

      const tinyPlane = 'TinyPlane';
      const tinyPlaneSize = this.gc.measureText(tinyPlane);
      this.gc.drawText((this.gc.getWidth() - tinyPlaneSize.width) >> 1, (this.gc.getHeight() - tinyPlaneSize.height) >> 1, tinyPlane.substring(0, Math.floor(timer / 5)));
      this.gc.display();
